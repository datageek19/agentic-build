...
│  │ (DS_ENT_BI)      │  │ WELLS_CONTEXT_SVC  │  │ DK_INSERT/UPDATE/etc.   │  │
│  │                   │  │ COLUMN_CATALOG_SVC │  │                         │  │
│  └────────┬──────────┘  └────────┬───────────┘  └─────────────────────────┘  │
└───────────┼───────────────────────┼──────────────────────────────────────────┘
            │                       │
...
│  │ (147 source views)     │  │ DOMAIN_KNOWLEDGE (5,249 rows)             │  │
...
| **Purpose** | Domain knowledge CRUD agent — conversational interface for managing DOMAIN_KNOWLEDGE entries |
...
- Cortex Search: `WELLS_CONTEXT_SVC` (domain knowledge retrieval)
...
- Cortex Search: `WELLS_CONTEXT_SVC`
...
- Backing: `DATASCIENCE_DEV.DS_ENT_EXP.WELLS_CONTEXT_SVC`
...
- `DATASCIENCE_DEV.DS_ENT_EXP.WELLS_CONTEXT_SVC` (Cortex Search)
...
| **Purpose** | Read-only admin querying of DOMAIN_KNOWLEDGE and DOMAIN_APP_ADMIN_USER tables. Used by DK_MANAGEMENT_AGENT. |
...
| Field | Value |
|-------|-------|
| **FQN** | `DATASCIENCE_DEV.DS_ENT_EXP.WELLS_CONTEXT_SVC` |
| **Search Column** | CONTEXT_PROFILE |
| **Source** | DOMAIN_KNOWLEDGE (active, non-null profile) |
| **Rows** | 5,214 |
| **Refresh** | 1 min |
...
References:
- `DATASCIENCE_DEV.DS_ENT_EXP.DOMAIN_KNOWLEDGE`
- `DATASCIENCE_DEV.DS_ENT_EXP.DOMAIN_APP_ADMIN_USER`
- `DATASCIENCE_DEV.DS_ENT_EXP.WELLS_CONTEXT_SVC` (DK_SEARCH_SIMILAR only)
...
| Table | Rows | Purpose |
|-------|------|---------|
| DOMAIN_KNOWLEDGE | 5,249 | Primary knowledge store (SYNONYM, RULE, METRIC, PATTERN, COLUMN_DESC, HIERARCHY, TABLE_DESC) |
...
| CONTEXT_PROFILE | VARCHAR | Pre-computed search text (indexed by WELLS_CONTEXT_SVC) |
...
| WELLVIEW_KNOWLEDGE_BASE_CURATED | 336 | Legacy V2 curated knowledge (superseded by DOMAIN_KNOWLEDGE) |
| WELLVIEW_KNOWLEDGE_BASE_V2 | 338 | Legacy V2 structured knowledge (superseded) |
| WELLVIEW_SYNONYM_MAP | 169 | Legacy V2 synonyms (superseded by DOMAIN_KNOWLEDGE SYNONYM entries) |
| WELLVIEW_AGENT_TEST_QUESTIONS | 92 | Test question suite for agent evaluation |
| DOMAIN_KNOWLEDGE_PRE_V2_SCOPE | 5,224 | Backup/snapshot of pre-expansion data |
...
| `DATASCIENCE_DEV_ADHOC_WH` | WELLS_CONTEXT_SVC refresh |
...
- [ ] `DOMAIN_KNOWLEDGE` (with data — export/import required)
...
- [ ] `WELLS_CONTEXT_SVC` (depends on DOMAIN_KNOWLEDGE)
...
- [ ] `SV_DOMAIN_KNOWLEDGE_ADMIN` (depends on DOMAIN_KNOWLEDGE + DOMAIN_APP_ADMIN_USER)

### Layer 5: Stored Procedures (depend on tables + search services)
- [ ] `DK_CHECK_ACCESS` (depends on DOMAIN_APP_ADMIN_USER)
- [ ] `DK_VALIDATE_ENTRY` (depends on DOMAIN_KNOWLEDGE)
- [ ] `DK_INSERT_ENTRY` (depends on DK_CHECK_ACCESS, DK_VALIDATE_ENTRY, DOMAIN_KNOWLEDGE)
- [ ] `DK_UPDATE_ENTRY` (depends on DK_CHECK_ACCESS, DOMAIN_KNOWLEDGE)
- [ ] `DK_DEACTIVATE_ENTRY` (depends on DK_CHECK_ACCESS, DOMAIN_KNOWLEDGE)
- [ ] `DK_BULK_DEACTIVATE` (depends on DK_CHECK_ACCESS, DOMAIN_KNOWLEDGE)
- [ ] `DK_SEARCH_SIMILAR` (depends on WELLS_CONTEXT_SVC)

### Layer 6: Agents (depend on everything above)
- [ ] `AGENT_WELLS_V3` (depends on SV_ENT_WELLS_*, WELLS_CONTEXT_SVC, WELLVIEW_COLUMN_CATALOG_SEARCH)
- [ ] `DK_MANAGEMENT_AGENT` (depends on SV_DOMAIN_KNOWLEDGE_ADMIN, WELLS_CONTEXT_SVC, all DK_* procedures)

### Layer 7: MCP Server (depends on semantic views + search services)
- [ ] `SF_MCP_ENT_WELLS` in target AI schema (depends on all SV_ENT_WELLS_* + WELLS_CONTEXT_SVC)
...
- [ ] `DK_MANAGER` Streamlit (depends on DOMAIN_KNOWLEDGE, DOMAIN_APP_ADMIN_USER, WELLS_CONTEXT_SVC)
...
|--------|--------------|-------|
| DOMAIN_KNOWLEDGE | `COPY INTO @stage` or CTAS | 5,249 rows; VARIANT and ARRAY columns |
...
GRANT SELECT ON TABLE <prod_db>.<schema>.DOMAIN_KNOWLEDGE TO ROLE <agent_role>;
